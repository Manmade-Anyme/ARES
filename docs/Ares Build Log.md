...
- **Nonzero Enforced Embargo Intervals**: Configured `WalkForwardPurgedCV` with a nonzero default `embargo_window = pd.Timedelta(minutes=15)` and explicit pipeline overrides (`config.market_movement_embargo_minutes = 15`, `config.trade_outcome_embargo_minutes = 30`). This prevents observations immediately following earlier test windows from leaking autoregressive serial correlation into subsequent training folds when default parameters are used.
