...
- **Nonzero Enforced Pre-Test Embargo Intervals**: Configured `WalkForwardPurgedCV` with a nonzero default `embargo_window = pd.Timedelta(minutes=15)` and explicit pipeline overrides (`config.market_movement_embargo_minutes = 15`, `config.trade_outcome_embargo_minutes = 30`). In expanding walk-forward folds, the embargo buffer is applied directly at the test boundary to exclude candidate training rows in `[test_start - embargo_window, test_start)` (as well as post-test rows in rolling slices), ensuring training features do not leak autoregressive serial correlation into evaluated test metrics used for model promotion.
- **Market-Only Scope for Stage 1 (Zero Inference Mismatch)**: Excluded `detector_scores` from `MarketMovementPipeline` input columns. Stage 1 is trained strictly on continuous market dynamics (`candle_features`, `volume_features`, `iv_features`, `oi_features`, `greek_features`, `structure_features`, `meta_features`), ensuring `stage1_feature_names` is 100% covered by `build_feature_vector()` at live serving time with zero missing or NaN detector columns.
- **Fold-Local Structural Feature Selection**: Strictly prohibited selecting Stage 2 structural features using full-dataset SHAP importance across walk-forward folds. Inside each fold $k$, features are selected from fold $k$'s pre-test Stage 1 model SHAP gain (or fixed *a priori* to canonical structural metrics), eliminating cross-boundary feature selection leakage.
- **Early Stopping Prohibition on Outer Test Partitions**: Strictly prohibited passing outer test sets to `model.fit()` as `eval_set` during walk-forward cross-validation. Outer folds must either disable early stopping (`early_stopping_rounds = None`) with fixed regularized shallow trees (`max_depth = 2`, `n_estimators = 50`), or carve an inner chronological validation split from `train_df`, ensuring outer test evaluation remains 100% out-of-sample.
- **Feature-Equality Snapshot Deduplication & Canonical Trade Identity**: Refined snapshot deduplication to verify feature equality across numeric feature vectors before dropping same-minute rows. Signaled rows (`TradeOutcomePipeline`) deduplicate strictly on canonical trade identity (`["trade_id", "feature_hash"]` / `["snapshot_uuid", "feature_hash"]`), ensuring distinct closed trades sharing a `signal_id` or with identical entry features are never collapsed. For continuous background snapshots (`MarketMovementPipeline`), true identical-feature duplicates are dropped, and intra-candle updates retain the canonical terminal snapshot (`keep="last"`) sorted chronologically by capture timestamp, preserving the finalized candle-close market state.
- **Workflow Primary Metrics Schema Preservation**: Guaranteed that `reports/ml/task183_offline_metrics.json` preserves all top-level keys expected by `.github/workflows/ml_training.yml:73-82` (`model_version`, `auc_roc` aliasing `mean_auc`, `sharpe_status`, `sharpe_annualized`, `sharpe_daily`, `sharpe_days`, `sharpe_trades`, `sharpe_window_start`, `sharpe_window_end`, `sharpe_total_pnl_points`, `shap_status`, `shap_computed`, `promoted`, `leakage_guard_passed`), eliminating `n/a` values in weekly Discord reports and job summaries.
- **Decisive Bidirectional Resolution Timestamp**: Refined resolution timestamping in `label_forward_points()` to prevent premature un-purging across split boundaries. Intermediate single-direction stops do not resolve bidirectional labels because the opposing direction can still reach its winning target. `resolution_timestamp` strictly records the decisive candle: the target hit candle for either direction, the second stop candle when both directions are irreversibly stopped, or the horizon candle on chop/timeout.
- **Deduplication Identifier Exclusion & Feature Hygiene**: Registered `trade_id`, `snapshot_uuid`, `signal_id`, `signal_setup_type`, and `resolution_timestamp` into `_META_COLS` in `ml_signal/dataset.py` and `FORBIDDEN_OUTCOME_FIELDS` in `ml_signal/leakage_guards.py`, guaranteeing that tracking identifiers and partition timestamps are dropped or excluded before constructing feature matrix $X$ and never enter XGBoost/LightGBM.
- **Positive Student's $t$ Critical Value for CI Lower Bound**: Formalized confidence interval calculation using positive critical value $t_{\text{crit}} = t_{0.975, K-1} > 0$ (`scipy.stats.t.ppf(0.975, df=evaluable_folds - 1)`), guaranteeing `ci_95_lower = mean - t_crit * se` is strictly below the mean and eliminating reversed endpoints.
- **Workflow CI Containment & Gate Enforcement**: Integrated `--enforce-gate` flag in `train_offline.py` and `.github/workflows/ml_training.yml`. Gate failures exit with code 1, immediately halting scheduled CI jobs. Git staging excludes `*_unpromoted.joblib` and rejection audits, and Fly.io deployment is gated on verified promotion (`needs.train.outputs.promoted == 'true'`).
- **Rolling / Inner-OOF Stage 1 Transfer Scoring**: Eliminated in-sample stacking lookahead leakage in hybrid walk-forward cross-validation. Outer test rows are scored using a Stage 1 market model trained strictly on historical snapshots prior to the fold's test start timestamp (`resolution_timestamp < T_k^{test_start}`). Outer training rows are scored via rolling / inner-OOF cross-fitting where each trade entry only sees Stage 1 snapshots resolved prior to that entry (`resolution_timestamp < t_entry`), preventing Stage 2 from training on future-informed Stage 1 probabilities. Only upon passing the promotion gate are final Stage 1 and Stage 2 models fitted on the full dataset to construct `HybridPredictorBundle`.
- **Unified Leakage-Gate Metric Name**: Standardized the data leakage verification metric key strictly on `leakage_guard_passed: bool` across validation output (`compute_cv_metrics()`), gate evaluation (`evaluate_promotion_gate()`), and metrics serialization, eliminating dynamic key mismatch.
- **Gated Promotion by Default & Scheduled Workflow Alignment**: Configured `train_offline.py` to default to gated promotion (`--promote`), strictly evaluated by `promotion_gate.py`. Passing candidates promote to canonical `v{N}.joblib` while failing candidates safely reject without touching production pointers (`_unpromoted.joblib`). Documented workflow migration for `.github/workflows/ml_training.yml` to explicitly invoke `python -m ml_signal.train_offline --promote`.
- **Single Production Target & Artifact Disambiguation**: Resolved promotion ambiguity under `--pipeline all --promote`. The Trade Outcome pipeline is designated the sole target eligible to promote to the canonical production slot `ml_signal/models/v{N}.joblib`, while the Market Movement model outputs auxiliary research artifacts (`models/market_movement_v{N}.joblib`), preventing trade-outcome reliability scoring from being overwritten by raw directional move models.
- **Timestamp-Anomaly Trade Exclusion**: Preserved MANM-152 integrity by explicitly rejecting/excluding trades where `time_metrics_excluded is True`, `exit_timestamp is None/NaT`, or `exit_timestamp <= entry_timestamp/timestamp` prior to purged walk-forward cross-validation.
- **Evaluable Fold Gating Invariant**: Replaced raw requested split checks (`n_splits >= 4`) with `evaluable_folds >= 4` and `degenerate_folds == 0`, ensuring folds with single-class test slices or undefined/NaN metrics cause rejection.
- **Post-Flattening Deduplication & Query Columns**: Added `signal_id` and `signal_setup_type` to `_fetch_ml_collection()` query, and specified deduplication on composite keys `["timestamp", "signal_id", "signal_setup_type"]` and fallback `["timestamp", "close"]` post-flattening.
- **Actual Label Resolution Timestamp Purging**: Replaced fixed timedelta purge window with exact observation resolution timestamps (`resolution_timestamp` in `MarketMovementPipeline`, `exit_timestamp` in `TradeOutcomePipeline`). Purging strictly enforces `assert train_df["resolution_timestamp"].max() < test_df["timestamp"].min()`, preventing leakage across irregular candle intervals or market collection gaps.
