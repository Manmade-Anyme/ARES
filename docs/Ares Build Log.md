...
- **Fold-Level Cross-Fitting for Stage 1 Transfer Feature**: Eliminated lookahead leakage in hybrid walk-forward cross-validation. Inside each outer fold, Stage 1 market model is trained strictly on historical snapshot observations prior to that fold's test start timestamp. Only upon passing the promotion gate are final Stage 1 and Stage 2 models fitted on the full dataset to construct `HybridPredictorBundle`.
