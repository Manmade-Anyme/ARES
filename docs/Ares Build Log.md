...
- **Timestamp Bucketing for Snapshot Deduplication**: Added canonical 1-minute candle flooring (`dt.floor("1min")`) prior to composite key deduplication, preventing duplicate polling snapshots with differing sub-minute seconds within the same minute cycle from inflating samples across fold boundaries.
