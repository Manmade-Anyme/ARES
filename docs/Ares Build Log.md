...
- **Feature-Equality Snapshot Deduplication & Canonical State Retention**: Refined snapshot deduplication to verify feature equality across numeric feature vectors before dropping same-minute rows. Prevents intra-candle polling cycles with updated OI, IV, volume, or Greeks from being falsely discarded as duplicate rows. Signaled rows deduplicate strictly on identical `signal_id` + feature equality; for continuous background snapshots, true identical-feature duplicates are dropped, and intra-candle updates retain the canonical terminal snapshot (`keep="last"`) sorted chronologically by capture timestamp, preserving the finalized candle-close market state.
