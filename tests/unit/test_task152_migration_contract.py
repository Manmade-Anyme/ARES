...
CUTOVER_MIGRATION = Path(
    "migrations/2026-09-12-task150-cutover-signal-uuid.sql"
).read_text()
...
def test_deferred_greenfield_cutover_persists_required_entry_timestamp():
    assert re.search(
        r"INSERT INTO active_trades \(.*?direction,\s+entry_timestamp,\s+entry_price,",
        CUTOVER_MIGRATION,
        re.DOTALL,
    )
    assert re.search(
        r"p_direction,\s+p_entry_timestamp,\s+p_entry_price",
        CUTOVER_MIGRATION,
    )
    assert CUTOVER_MIGRATION.count(
        "entry_timestamp IS NOT DISTINCT FROM p_entry_timestamp"
    ) >= 2
