...
3. Consecutive qualifying observations of the same key increment persistence. At three observations, the bias becomes `PERSISTENT`, but persistence alone is not enough for entry.
4. The filter records the first wall interaction when a candle range enters `oi_wall_initial_interaction_distance_pts`. A defended-side close records the interaction; a wrong-side close expires the bias. This first interaction can never be the re-test entry.
5. On a later candle only, the filter requires a favourable excursion of at least `oi_wall_min_excursion_pts` from the wall strike: below the CE wall for bearish bias or above the PE wall for bullish bias. Only then does the state become `RETEST_READY`.
...
| `PERSISTENT` | first wall interaction, defended-side close | `INTERACTED` | no |
| `INTERACTED` | later excursion >= configured minimum | `RETEST_READY` | no |
...
- The filter never consumes a wall from `update()`. Every `QUALIFIED` decision is acknowledged exactly once after the engine's final gates; no signal alert or cooldown is attached to a merely provisional decision.
