...
4. The filter records the first wall interaction whenever it occurs, including while persistence is `TRACKING` on snapshot 1 or 2. A candle range entering `oi_wall_test_distance` followed by a defended-side directional close (bearish for CE, bullish for PE, MANM-184) records the interaction; a wrong-side close expires the bias. This first interaction can never be the re-test entry, and its timestamp/price remain attached as persistence continues.
