...
def snapshot(..., signal=None, trade_id: str | None = None,
...
MLCollector.snapshot(..., signal=signal, trade_id=entry.trade_id,
...
| `main.py` | Gate trade creation and ML binding on successful signal persistence; pass the returned trade ID and binding status to the snapshot; await entry commit before `update_trades`. |
...
- **Fast exits:** use the atomic entry transaction and retain bounded exit retry/reconciliation with terminal telemetry.
